...
wd.get_weather("kolkata")
# /e:/programming/python/weatherApp/gui.py
# A modern-looking weather GUI using tkinter with placeholder data.
# Run: python gui.py
...
# -------------------------
def fake_weather_for_city(city: str):
    """Return dummy weather data for a city (placeholder)."""
    # Generate plausible-looking random values
    temp = random.randint(-5, 36)
    feels_like = temp + random.choice((-2, -1, 0, 1, 2))
    humidity = random.randint(20, 95)
    wind_kph = round(random.uniform(0.5, 25.0), 1)
    condition = random.choice(["Sunny", "Partly Cloudy", "Cloudy", "Rain", "Snow"])
    return {
        "city": city.title(),
        "temp": f"{temp}°C",
...
class WeatherApp(tk.Tk):
    def __init__(self):
        ...
        self.update_weather(fake_weather_for_city("San Francisco"))
    ...
    def _create_main_panels(self):
        # Main content area: temperature box (left) and small info box (right)
        main_frame = tk.Frame(self.container, bg=BG_COLOR)
        main_frame.pack(fill="both", expand=True)

        # Left - Large Temperature Box
        left_frame = tk.Frame(main_frame, bg=BG_COLOR)
        left_frame.pack(side="left", fill="both", expand=True)

        # Slight drop shadow imitation
        ...
        # Right - small box with other data
    ...
    def update_weather(self, data: dict):
        """Populate UI with provided data dict (placeholder)."""
    ...
    def _on_search(self, event=None):
        ...
        # If placeholder text present, treat as empty
        if not query or query.startswith("Enter city"):
            # simple visual feedback: flash the entry background
            ...
            return
        # Instead of calling a real API, we generate fake data
        data = fake_weather_for_city(query)
